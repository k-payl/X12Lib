...
void Resize(HWND hwnd, engine::WINDOW_MESSAGE type, uint32_t param1, uint32_t param2, void* data);
void Init();
void Render();

int APIENTRY wWinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPWSTR, _In_ int)
{
	engine::Dx11GpuProfiler* gpuprofiler = new engine::Dx11GpuProfiler(vec4(0.6f, 0.6f, 0.6f, 1.0f), 0.0f);
	...
	res = new Resources{};
	core->Init(engine::INIT_FLAGS::NO_CONSOLE, gpuprofiler, &dx11::InitDX11);
	...
	engine::MainWindow* window = core->GetWindow();
...
void Render()
{
	...
	//engine::RenderProfiler(frameGPU, frameCPU);
...
void Resize(HWND hwnd, engine::WINDOW_MESSAGE type, uint32_t param1, uint32_t param2, void* pData)
{
	if (type != engine::WINDOW_MESSAGE::SIZE)
		return;
