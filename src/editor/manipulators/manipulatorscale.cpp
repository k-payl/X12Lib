...
static math::mat4 CubeTranslateMatDefault[3];
static math::mat4 CubeTranslateMat[3];
static math::mat4 AxesCorrectionMat[3];
static math::mat4 AxesCorrectionMatDefault[3];
...
void ManipulatorScale::render(const CameraData &cam, const math::mat4 &selectionTransform, const QRect &screen)
{
	/*
	...
	math::mat4 *CubeTranslatePtr;
	math::mat4 *AxesCorrectionPtr;
	...
	for (int i = 0; i < 3; i++)
	{
		math::mat4 MVP = cam.ViewProjMat * selectionTransform * distanceScaleMat * AxesCorrectionPtr[i];
	...
	for(int i = 0; i < 3; i++)
	{
		...
		math::mat4 scale = math::mat4{0.05f};
		math::mat4 MVP = cam.ViewProjMat * selectionTransform * distanceScaleMat * CubeTranslatePtr[i] * scale;
		shader->SetMat4Parameter("MVP", &MVP);
...
void ManipulatorScale::updateMouse(const CameraData &cam, const math::mat4 &selectionTransform, const QRect &screen, const math::vec2 &normalizedMousePos)
{
	math::vec3 center = selectionTransform.Column3(3);
	...
	if (state == STATE::SCALING_ARROW_HANDLE)
	{
		...
		{
			math::vec3 pos = movesAlongLine.projectPoint(I.worldPos);
			...
			math::vec3 newScale = originalLocalScale * scaleVector;
...
bool ManipulatorScale::isMouseIntersect(const math::vec2 &normalizedMousePos)
{
	return underMouse != MANIPULATOR_ELEMENT::NONE;
}

void ManipulatorScale::mousePress(const CameraData &cam, const math::mat4 &selectionTransform, const QRect &screen, const math::vec2 &normalizedMousePos)
{
