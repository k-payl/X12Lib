	...
	QWidget(parent, Qt::MSWindowsOwnDC)
{
	...
	if (0) // comment for custom drawing
	{
	...
	camPos = math::vec3(0.0f, -4.0f, 1.8f);
	camRot = math::quat(70.0f, 0.0f, 0.0f);
}

void RenderWidget::onEngineInited(engine::Core *c)
{
	core = c;
...
void RenderWidget::onEngineFree(engine::Core *c)
{
	//meshPlane.release();
	core = nullptr;
}
...
void RenderWidget::mouseMoveEvent(QMouseEvent *event)
{
	...
	int w = size().width();
	int h = size().height();
	normalizedMousePos = math::vec2((float)mousePos.x() / w, (float)(h - mousePos.y()) / h);
...
/*
	...
	compositeTransform(data.WorldTransform, camPos, camRot, math::vec3(1.0f, 1.0f, 1.0f));
...
/*
	...
	{
		engine::GameObject *selected = editor->FirstSelectedObjects();
		if (selected->GetType() == OBJECT_TYPE::MODEL)
		{
			numSelected = 1;
			selectedModel = reinterpret_cast<engine::Model*>(selected);
		}
	}
	...
	if (leftMouseClick && !keyAlt)
	{
			...
			|| !manipulator)
		{
			...
			engine::GameObject *ob = resMan->FindObjectById((int)id);
...
//				qDebug() << "game obejctnot found";
			...
			QSet<engine::GameObject*> slected;
			if (ob)
				slected.insert(ob);
...
/*
	...
	{
		math::vec3 dist = camPos - focusWorldCenter;
		...
		if (leftMouseDown) // orbit
		{
			...
			newRotMat.Transpose();
			camRot = math::quat(newRotMat);
		}
	}
	else if (rightMousePressed) // free moving
	{
		math::vec3 orthDirection = math::vec3(cam.WorldTransform.Column(0)); // X local
		math::vec3 forwardDirection = -math::vec3(cam.WorldTransform.Column(2)); // -Z local
		math::vec3 upDirection = math::vec3(0.0f, 0.0f, 1.0f); // Z world
		...
		math::quat dxRot = math::quat(-deltaMousePos.y() * dt * RotateSpeed, 0.0f, 0.0f);
		math::quat dyRot = math::quat(0.0f, 0.0f,-deltaMousePos.x() * dt * RotateSpeed);
...
void RenderWidget::onFocus(const math::vec3 &worldCenter)
{
	CameraData cam;
	if (!calculateCameraData(cam))
		return;
