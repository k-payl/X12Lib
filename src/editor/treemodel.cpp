...
bool TreeModel::dropMimeData(const QMimeData *mimeData, Qt::DropAction action, int row, int column, const QModelIndex &parent)
{
	...
	engine::GameObject *parentObj = parentNode->obj();
	...
	for (int i = 0; i < count; ++i) {
		...
		engine::GameObject *obj = node->obj();
...
QVariant TreeModel::data(const QModelIndex &index, int role) const
{
	...
	{
		engine::GameObject *obj = node->obj();
		if (obj->GetType() == engine::OBJECT_TYPE::CAMERA)
			return QIcon(CAMERA_ICON);
		else if (obj->GetType() == engine::OBJECT_TYPE::LIGHT)
