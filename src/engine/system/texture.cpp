...
x12::ICoreTexture* createFromDDS(std::unique_ptr<uint8_t[]> dataPtr, size_t size, x12::TEXTURE_CREATE_FLAGS flags);
std::unique_ptr<uint8_t[]> convertRGBtoRGBA(const uint8_t* dataIn, uint32_t w, uint32_t h, bool hasMipmaps, bool bgrTorgb);
...
x12::ICoreTexture* createFromDDS(std::unique_ptr<uint8_t[]> dataPtr, size_t size, x12::TEXTURE_CREATE_FLAGS flags)
{
	...
	x12::TEXTURE_TYPE type = x12::TEXTURE_TYPE::TYPE_2D;
	x12::TEXTURE_FORMAT format = x12::TEXTURE_FORMAT::UNKNOWN;
	...
	else
	{
		...
		if (ddpf.flags & DDS_RGB)
		{
			...
			case 32:
				...
				{
					newData = unique_ptr<uint8_t[]>(new uint8_t[sizeInBytes]);
					memset(newData.get(), 255, sizeInBytes);

					uint8_t* ptrSrc = imageData;
...
std::unique_ptr<uint8_t[]> convertRGBtoRGBA(const uint8_t* dataIn, uint32_t w, uint32_t h, bool hasMipmaps, bool bgrTorgb)
{
	...
	unique_ptr<uint8_t[]> newData(new uint8_t[sizeInBytes]);
	memset(newData.get(), 255, sizeInBytes);
